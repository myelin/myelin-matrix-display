...
Adafruit_WS2801_PP::Adafruit_WS2801_PP(uint16_t n, uint8_t order) {
  rgb_order = order;
  alloc(n);
  updatePins();
}
...
Adafruit_WS2801_PP::Adafruit_WS2801_PP(uint16_t n, uint8_t dpin, uint8_t cpin, uint8_t order) {
  rgb_order = order;
  alloc(n);
...
void Adafruit_WS2801_PP::alloc(uint16_t n) {
  begun   = false;
...
Adafruit_WS2801_PP::Adafruit_WS2801_PP(void) {
  begun     = false;
  numLEDs   = 0;
  pixels    = NULL;
...
void Adafruit_WS2801_PP::begin(void) {
  if(hardwareSPI == true) {
    startSPI();
  } else {
...
void Adafruit_WS2801_PP::updatePins(void) {
  hardwareSPI = true;
  datapin     = clkpin = 0;
...
void Adafruit_WS2801_PP::updatePins(uint8_t dpin, uint8_t cpin) {
...
void Adafruit_WS2801_PP::startSPI(void) {
    SPI.begin();
...
uint16_t Adafruit_WS2801_PP::numPixels(void) {
  return numLEDs;
}

// Change strand length (see notes with empty constructor, above):
void Adafruit_WS2801_PP::updateLength(uint16_t n) {
...
void Adafruit_WS2801_PP::setPixelColor(uint16_t n, uint8_t r, uint8_t g, uint8_t b) {
...
void Adafruit_WS2801_PP::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) { // Arrays are 0-indexed, thus NOT '<='
    uint8_t *p = &pixels[n * 3];
...
uint32_t Adafruit_WS2801_PP::getPixelColor(uint16_t n) {
  if(n < numLEDs) {
    uint16_t ofs = n * 3;
    // To keep the show() loop as simple & fast as possible, the
    // internal color representation is native to different pixel
